...
public class BookingPane extends BasicPane {
	...
	private void fillNameList() {
		...
		db.getMovies(nameListModel);

	}
	/*
	 * private void fillNameList(){ nameListModel.removeAllElements();
	 * 
	 * String query = "SELECT distinct movie FROM shows"; ResultSet rs =
	 * db.execute(query); while(rs.next()){
	 * nameListModel.addElement(rs.getString("movie")); } }
	 */
	...
	private void fillDateList(String movieName) {
		...
		db.getDateList(dateListModel, movieName);
	...
	class DateSelectionListener implements ListSelectionListener {
		...
		public void valueChanged(ListSelectionEvent e) {
			...
			String theaterName = db.getTheaterName(movieName, date);
			String seatsLeft = db.getSeatsLeft(movieName, date);

			fields[0].setText(movieName);
			fields[1].setText(date);
			fields[2].setText(theaterName);
			fields[3].setText(seatsLeft);
